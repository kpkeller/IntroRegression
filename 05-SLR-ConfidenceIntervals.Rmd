...
To create a confidence interval, we *invert* a hypothesis test. Recall that for a hypothesis test,  we computed
$$t = \frac{\hat\beta_1 - \beta_{10}}{\widehat{se}(\hat\beta_1)}$$
by plugging in $\hat\beta_1$, $\widehat{se}(\hat\beta_1)$, and $\beta_{10}$.
...
The distribution of  $t = \dfrac{\hat\beta_1 - \beta_{1}}{\widehat{se}(\hat\beta_1)}$ is $T_{n-2}$. This distribution has mean zero and a standardized variance (it's close to 1, although not exactly 1). There exists a number, which we denote $t_{\alpha/2}$ such that the area between them is $1-\alpha$. Mathematically, this can be written:
...
rocket <- read_csv(paste0(data_dir, "rocket.csv"))
```


```{r eval=FALSE, echo=F}
g_rocket <- ggplot(rocket)  + theme_bw()  +
  geom_point(aes(x=age, y=shear_strength)) +
  ylab("Shear Strength (psi)") + 
  xlab("Age (weeks)")
g_rocket + geom_smooth(aes(x=age, y=shear_strength), method="lm", se=FALSE)
```
...
rocket_lm <- lm(shear_strength~age, data=rocket)

alpha <- 0.05
t_alphaOver2 <- qt(1-alpha/2, df = nobs(rocket_lm)-2)
t_alphaOver2

CI95Lower <- coef(rocket_lm)[2] - t_alphaOver2 * tidy(rocket_lm)$std.error[2]
CI95Upper <- coef(rocket_lm)[2] + t_alphaOver2 * tidy(rocket_lm)$std.error[2]
