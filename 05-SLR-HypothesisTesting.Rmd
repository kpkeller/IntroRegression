...
We reject the null hypothesis that there is no difference in body mass beteen female and male penguins.
...
```{example peng-lm-intro-testingconclusion}
We can update the interpretation summary from Example \@ref(exm:peng-lm-intro) by adding a second sentence:
```
  
*A difference of one mm in flipper length is  associated with an estimated difference of 49.7 g greater average body mass among penguins in Antarctica. We reject the null hypothesis that there is no linear relationship between flipper length and average penguin body mass ($p < 0.0001$).* 
