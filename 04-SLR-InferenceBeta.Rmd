...
$$t = \frac{49.7 - 40}{\sqrt{1.52}} = 7.9$$

In R, this can be computed as:

```{r echo=TRUE}
penguin_lm_df <- tidy(penguin_lm)
t <- (penguin_lm_df$estimate[2]- 40)/sqrt(penguin_lm_df$std.error[2])
t
```
...
In this example, $p \approx 1$, so we fail to reject the null hypothesis that $\beta_1$ is greater than or equal to zero.
