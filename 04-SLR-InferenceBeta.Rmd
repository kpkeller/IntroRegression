...
## Not Regression: One-sample $t$-test
...
Compare the following output with the results calculated manually the previous subsection.
...
## Summarizing Inference for $\beta_1$
...
Confidence intervals are generally reported in parantheses after a point esitmate is given. It is standard to specify the confidence level when doing so.
...
*We reject the null hypothesis that there is no difference in body mass beteen female and male penguins ($p < 0.0001$). The estimated difference in average body mass of penguins, comparing males to females, is 683 grams (95% CI: 526, 840), with males having larger average mass.* 
