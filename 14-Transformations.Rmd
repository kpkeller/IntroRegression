...
```{r g-housing-inc-price-pretrans, echo=FALSE, fig.cap="Median single-famly residence prices and median annual household income in U.S. metropolitan areas.", message=FALSE, out.width="80%",  fig.width=5, fig.height=3.5}
...
```{r g-housing-inc-price-resid, echo=FALSE, fig.cap="Residuals from SLR model of median single-famly residence prices as a function of median annual household income in U.S. metropolitan areas.", message=FALSE, out.width="80%",  fig.width=5, fig.height=3.5}
...
ggplot() + theme_bw() + 
  ...
  geom_smooth(aes(x=fitted(housing_lm), y=rstandard(housing_lm)), se=F, col="purple") +
...
```{r g-housing-inc-price-logy, echo=FALSE, fig.cap="Logarithm of median single-famly residence prices and median annual household income in U.S. metropolitan areas.", message=FALSE, out.width="80%",  fig.width=5, fig.height=3.5}
...
```{r g-housing-inc-price-logy-resid, echo=FALSE, fig.cap="Residuals from SLR model of logarithm of median single-famly residence prices as a function of median annual household income in U.S. metropolitan areas.", message=FALSE, out.width="80%",  fig.width=5, fig.height=3.5}
...
ggplot() + theme_bw() + 
  ...
  geom_smooth(aes(x=fitted(housing_lm2), y=rstandard(housing_lm2)), se=F, col="purple") +
  xlab("Fitted Values") + ylab("Standardized Residuals") # + ggtitle("Synthetic Data 2: Residuals v. Fitted")
```

Notice how in Figure \@ref(fig:g-housing-inc-price-logy-resid), the values on the left hand side are more evenly spread out vertically than in Figure \@ref(fig:g-housing-inc-price-resid). The logarithm transform of the housing prices leads to less heteroscedasticity in the residuals.  The trend line in Figure \@ref(fig:g-housing-inc-price-resid) also shows evidence of non-linearity, while the trend line in Figure \@ref(fig:g-housing-inc-price-logy-resid) is closer to being flat, indicating less violation of the linearity assumption.

::::



### Interpreting a Model with $\log Y$
...
If the function $f()$ is chosen to be the logarithm, then we can interpret $\beta_j$ in terms of the geometric mean of $Y$. Scientifically, using $\log(Y_i)$ in place of $Y_i$ is often appropriate for outcomes that operate on a multiplicative scale. Concentrations of pollution in the air or biomarkers in the blood are two examples of this.
...
The geometric mean is only define for positive numbers and is always smaller than the arithmetic mean (the usual "average"). It has the advantage of being more robust to outliers than the arithmetic mean and is also useful when there are multiplicative changes in a variable.

To connect the geometric mean to regression models with $\log(Y)$ as the outcome, we first rewrite the geometric mean as:
\begin{align*}
...
\end{align*}
This relationship shows that $\exp(\E[\log(X)])$ can be interpreted
as the geometric mean of a random variable \(X\). 
...
In addition to transforming only $x$ or only $Y$, we can fit a model with a transform applied to both. This is most commonly done using a logarithm transform on both $x$ and $Y$. The SLR model becomes
...
By exponentiating both sides of \@ref(eq:logylogx), we obtain
\begin{align}
e^{\E[\log(Y_i)]} &= e^{\beta_0}e^{\beta_1\log(x_i)} =e^{\beta_0}x_i^{\beta_1}
(\#eq:explogyexplogx)
\end{align}
Equation \@ref(eq:explogyexplogx) shows that $\exp(\beta_1)$ is the multiplicative difference in the geometric mean of $Y$ for an $e$-fold difference in $x$. Conceptually, this means that multiplicative differences in $x$ are related to multiplicative differences in (the geometric mean of) $Y$. Thus, the log-log model \@ref(eq:logylogx) can work particularly well for an exponential or growth model.
...
```{example}
Figure \@ref(fig:sp500-price-cap2) shows the stock price and market capitalization for companies on the S&P 500. The data are concentrated in the bottom left, with only a few companies with large market capitalizations or high stock prices. Based on this scatterplot, a linear regression model that uses these values (market capitalization and stock price) directly would not fit the data well.
```
...
```{r sp500-price-cap2, eval=TRUE,  echo=FALSE, fig.cap="Stock prices and market capitalization.", message=FALSE, out.width="75%", fig.width=5, fig.height=3.5}
...
```{r sp500-price-cap-logxy, eval=TRUE, echo=FALSE, fig.cap="Stock prices and market capitalization, after log-transforming both variables. The red line is shows the SLR line and the blue curve is a smooth through the data.", message=FALSE, out.width="75%", fig.width=5, fig.height=3.5}
...
We can estimate the model coefficients by fitting the model:
...
```{r eval=F, echo=F}
library(pixiedust)
dust(sp_lm1, conf.int=TRUE) %>% 
  sprinkle(cols = c("estimate", "std.error", "statistic", "conf.low", "conf.high"),
           round = 3) %>% 
  ...
  sprinkle_colnames(term = "Term",
                    ...
                    std.error = "Std Err",
                    statistic = "Statistic",
                    conf.low = "CI Lower Limit",
                    conf.high = "CI Upper Limit",
                    p.value = "P-value") %>%
  kable()
```
...
```{r eval=TRUE, out.width="75%", echo=FALSE, fig.height=3.5, fig.width=5, message=FALSE}
...
To estimate coefficients for a polynomial model, quadratic (and higher-order) terms can be added directly to the model, like in \@ref(eq:lmquad). This can be accomplished in `R` by adding the terms `I(x^2)` to the model formula. (The `I()` is necessary to indicate that you are performing a transformation of the variable `x`). 

:::: {.examplebox}

```{example}
We can fit a quadratic model to the data from Example \@ref(exm:fuel-efficiency-quad1) by running the code:
```  
...
```{example}
In our fuel efficiency data example, we can fit an equivalent model using the code:
```
...
```{example fueleffic-cubic}
Suppose we fit a cubic model to the fuel efficiency data. We can test whether the cubic model is a better fit compared to the quadratic model using the null hypothesis $H_0: \beta_3 = 0$. There are two equivalent ways to test this: (i) looking at the $t$-test corresponding to $\beta_3$ directly and (ii) using an F-test to compare the cubic model to the quadratic model.
```

```{r echo=TRUE, size="small"}
mpg_cubic <- lm(hwy~poly(displ, 3), data=mpg)
tidy(mpg_cubic)
```

From the `tidy()` output, we have do not have evidence to reject the null hypothesis that $\beta_3 = 0$ ($p=0.052$).  We would obtain the same result using an F-test:

```{r echo=TRUE, size="small"}
anova(mpg_quad_v2, mpg_cubic)
```
...
```{r spline-df4, echo=FALSE,  eval=TRUE, fig.cap="Natural cubic splines with $df=4$."}
library(splines)
...
```{exercise}
Compare the cubic model from Example \@ref(exm:fueleffic-cubic) to a model that includes engine displacement as only a linear term. State the null and alternative hypotheses, the test statistic, $p$-value, and a conclusion statement.
```