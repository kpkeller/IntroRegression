...
To compute the mean value for a combination of predictor variables, we simply plug those values into the MLR equation: 
...
To find the difference in mean value of the outcome between two observations, we can compute the mean for each and subtract. 
...
Suppose we have data on several football teams. We fit an MLR model with:
```
...
for all of the teams in the league. In the context of this model, provide an interpretation for each of the following:
...
```{exercise}
Using the model from Exercise \@ref(exr:football-mlr-interp), what is the mean number of point scored for a team that had 130 rushing yards, 75 passing yards, and 2 turnovers?
```
