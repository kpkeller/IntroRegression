...
From equation \@ref(eq:eq:mlr2), we can construct two different regression lines. For female penguins, $x_{i2} = 0$, so \@ref(eq:eq:mlr2) reduces to:
...
For male penguins, $x_{i2} = 0$, so \@ref(eq:eq:mlr2) reduces to:
\begin{align*}
...
```{r g-penguin-flip-mass-bysex-lm, echo=FALSE, fig.cap="Flipper length and body mass in the Palmer Penguin dataset."}
g_penguin_flip_mass +
...
```{example}
According to the MLR model \@ref(ex:mlr_x2}, what is the difference in average body mass between 
