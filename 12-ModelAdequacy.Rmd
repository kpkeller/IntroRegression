# Assessing Model Assumptions {#sec:modadequacy}
...
The residuals for a model are the difference between the predicted value and the true observation. Each residual gives an estimate of the error $\epsilon_i$. Collectively, the residuals can be used to assess violations of the model assumptions, as we will see below.
...
Raw residuals are the "regular" residuals:
$$e_i = y_i - \hat y_i$$
These provide a direct measure of the difference between the fitted values and the observations.
...
<!--                   y=hwy)) + -->
<!--    geom_smooth(aes(x=displ, y=hwy), method="lm", se=F) -->
<!-- ``` -->

<!-- ```{r echo=TRUE, size="footnotesize"} -->
<!-- mpg_lm <- lm(hwy~displ,data=mpg) -->
<!-- ggplot() + theme_bw() +  -->
<!--    geom_histogram(aes(x=residuals(mpg_lm)), bins=25) -->
<!-- ``` -->
...
```{r fit-without-point, echo=FALSE, message=FALSE}
...
### **Assumption 1**: $\E[\bmepsilon] = \mathbf{0}$  
...
To answer this, let's first plot the data with the regression line. Figure \@ref(fig:sp500-price-cap) shows the data, and it is clearly evident that the values of both variables are skewed. (In Section \@ref(), we'll revisit this data to see how a transformation can fix this).
