# Sums of Squares and F-Tests {#slrFtest}
...
$SS_{tot} = \displaystyle\sum_{i=1}^n(y_i - \overline{y})^2$ is the *total sum of squares*
...
&= \sum_{i=1}^n (y_i - \hat y_i)^2 +  \sum_{i=1}^n (\hat y_i - \overline y)^2\\
&= SS_{res} + SS_{reg}
\end{align*}
...
$SS_{tot} = \displaystyle\sum_{i=1}^n(y_i - \overline{y})^2$ is total variability in the $y$'s
...
$SS_{res} = \sum_{i=1}^n (y_i - \hat y_i)^2$ is variability "left over" from fitted regression model
...
$SS_{reg} = \sum_{i=1}^n (\hat y_i - \overline{y})^2$ is variability explained by the fitted regression model
