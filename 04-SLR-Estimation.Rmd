...
The task of computing $\hat\beta_0$ and $\hat\beta_1$ is commonly called "fitting" the model. Although we could explicitly compuate the values using equations \@ref(eq:hatbeta0) and \@ref(eq:hatbeta1), it is much simpler to let R do the computations for us.
...
* The estimated average body mass for female penguins is 4,546 grams.
...
$$\hat\beta_1 = \frac{S_{xy}}{S_{xx}} = \sum_{i=1}^n \frac{ (x_i - \overline{x})^2}{S_{xx}}y_i = \sum_{i=1}^n c_iy_i$$
...
Consider the housing price data introduced in Section \@ref(housingprices), with housing price as the outcome and median income as the predictor variable. Find the OLS for a simple linear regression analysis of this data and provide a 1-sentence interpretation for each.
```

```{exercise}
Compute $\hat\sigma^2$ for the model in Exercise \@ref(exr:housing-lm-coef) using the formulas. Compare to the value calculated from the `lm` object.
```
