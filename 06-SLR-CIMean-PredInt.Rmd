...
Confidence intervals for $\hat\mu_{Y | x_0}$ can be constructed in the same way as for the $\beta$'s:
...
```{r g-penguin-flip-mass-lm2, echo=F, message=FALSE}
...
```{r}
g_penguin_flip_mass_lm + geom_point(aes(x=x0, y=muhat), col="blue", size=3)
```
...
```{r eval=TRUE, echo=TRUE, size="footnotesize"}
predict(penguin_lm,
        newdata=data.frame(flipper_length_mm=c(50, 70, 90, 100)),
...
```{r eval=FALSE, echo=TRUE, warning=F, message=F, size="footnotesize"}
g_penguin_flip_mass + 
