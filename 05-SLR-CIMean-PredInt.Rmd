...
To make the plot by hand, we first need to construct a sequence of predictor values, compute the mean and corresponding CI for those values, and then at to a plot. This is demonstrated in the following code:
...
body_mass_ci_df <- data.frame(body_mass_ci,
                              flipper_length_mm= flipper_length_pred_df$flipper_length_mm)
...
The following code shows how to use equation \@ref(eq:varyhat) directly:
...
To use `predict()`, set `interval="prediction"` instead of `"confidence"`:
